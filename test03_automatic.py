...
for filename in filenames:
    if os.path.exists(os.path.join('D:\\TFG_Datos', filename)):
...
H = HDAS('D:\\TFG_Datos', existing_files_names)
...
fin_punto_espacial = 106  # Punto espacial final (exclusivo)
...
for i in range(inicio_punto_espacial, fin_punto_espacial):
    ...
    output_filename = f"d_{first_datetime_clean[:8]}_{first_datetime_clean[8:]}_{last_datetime_clean[8:]}_SP{iteration_number}_5.mat"

    # Construye el nombre del diccionario con el número de iteración
        
    features_name = f"d_{first_datetime_clean[:8]}_{first_datetime_clean[8:]}_{last_datetime_clean[8:]}_SP{iteration_number}_5"
